...
export default function useDeviceAttributes( attributes, setAttributes ) {
    ...
    const deviceName = 'Desktop' !== device ? device : '';

    const deviceAttributes = useMemo( () => splitAttributes( attributes ), [ JSON.stringify( attributes ) ] );

    const setDeviceAttributes = useMemo(
        () =>
            ( attrs = {}, objName = '' ) => {
                if ( objName ) {
                    setAttributes( {
