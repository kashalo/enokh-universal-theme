...
const FlexDirectionControls = ( props: FlexDirectionControls ): JSX.Element => {
    const { value, onChange, onReverse, label, directionValue, fallback } = props;
    ...
    return (
        <BaseControl
            ...
            className={ classnames( {
                [ `enokh-blocks-flex-direction-flexDirection-${ directionValue }` ]: true,
            } ) }
        >
            <ButtonGroup id={ 'flexDirection' } className="enokh-blocks-flex-button-group">
                { flexOptions.flexDirection.map( ( flexOption, index ) => {
                    return <Fragment key={ label + index }>{ ButtonElement( flexOption ) }</Fragment>;
                } ) }

                <Tooltip text={ __( 'Reverse', 'enokh-blocks' ) }>
                    <Button
