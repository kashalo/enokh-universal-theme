...
export default function useOutdentListItem() {
    const registry: any = useRegistry();
    const { moveBlocksToPosition, removeBlock, insertBlock, updateBlockListSettings } = useDispatch( blockEditorStore );
    const {
    ...
    function getParentListItemId( id ) {
        ...
        if ( getBlockName( parentListItemId ) !== 'enokh-blocks/list-item' ) {
            return;
        }
    ...
    return useCallback( ( clientIds = getSelectedBlockClientIds() ) => {
        ...
        if ( getBlockName( firstClientId ) !== 'enokh-blocks/list-item' ) {
            return;
        }
