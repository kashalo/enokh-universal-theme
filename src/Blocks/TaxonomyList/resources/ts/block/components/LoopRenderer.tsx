...
export default function LoopRenderer( props ) {
    ...
    return (
        dataContexts &&
        dataContexts.map( ( postContext: any ) => (
            <BlockContextProvider key={ postContext.termId } value={ postContext }>
                { postContext.termId === ( activeContextId || dataContexts[ 0 ]?.termId ) ? (
                    <>
                ...
                <MemoizedBlockPreview
                    ...
                    contextId={ postContext.termId }
                    setActiveContextId={ setActiveContextId }
                    isHidden={ postContext.termId === ( activeContextId || dataContexts[ 0 ]?.termId ) }
                />
            </BlockContextProvider>
        ) )
    );
